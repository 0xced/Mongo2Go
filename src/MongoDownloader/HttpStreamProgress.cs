...
internal class HttpStreamProgress : IDisposable
{
    ...
    public async Task<HttpStream> GetHttpStreamAsync(CancellationToken cancellationToken)
    {
        ...
        _contentLength = httpStream.IsStreamLengthAvailable ? httpStream.StreamLength : null;
    ...
    public long? BytesSaved => _contentLength.HasValue ? Math.Max(0, _contentLength.Value - _bytesTransferred) : null;
